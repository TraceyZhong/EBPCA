...
# and square symmetric, rotationally invariant noise
...
def compute_moments(X,k,remove=1):
  eigs = np.linalg.eigvalsh(X)[:-remove]
  m = []
  for j in range(k+1):
...
#
def AMP_free(X,u,innerprod,iters=10,rank=1,reg=0.001,plot_prefix=None):
  ...
  # Initialize U, F, mu, Delta, Phi
  U = np.reshape(u,(-1,1))
  F = np.zeros(shape=(n,0))
  mu = np.array([])
