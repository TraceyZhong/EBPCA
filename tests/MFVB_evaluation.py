...
for subset_size in subset_sizes:
    ...
    for n_copy in range(1, 51, 1):
        ...
        np.save('results/%s/EBPCA_error_across_iterations_summary_size_%i_n_rep_50.npy' % (data_name, subset_size), errors_by_iters)
...
for RMT_ini in [True, False]:
    for subset_size in subset_sizes:
        ...
        for n_copy in range(1, 51, 1):
            ...
            # joint estimation errors across iterations
            errors_by_iters.append([get_space_distance(PC_est[:, :, i], V_star) for i in range(6)])
            np.save('results/%s/MF-VB_RMT_%s_error_across_iterations_summary_size_%i_n_rep_50.npy' % (data_name, RMT_ini, subset_size), errors_by_iters)
